...
class OfficialLeaveWidget extends StatefulWidget {
  final AppUser user;
  ...
  @override
  State<OfficialLeaveWidget> createState() => _OfficialLeaveWidgetState();
}

class _OfficialLeaveWidgetState extends State<OfficialLeaveWidget> {
  @override
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        '• No deduction from leave balance (official work)\n'
                        '• Requires management approval before leave\n'
                        '• For office work outside office premises\n'
                        '• Can be applied multiple times as needed\n'
                        '• Sundays cannot be selected (non-working day)\n'
                        '• Single day: Today + 2 upcoming working days\n'
                        '• Multiple days: Up to 30 days in advance\n'
                        '• Edit/cancel allowed until applied date starts',
                        style: TextStyle(
                          fontSize: 11,
                          color: Colors.orange,
                        ),
  ...
  Widget _buildSingleDaySelector() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        InkWell(
          onTap: () => _selectOfficialLeaveDate(false),
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
  ...
  Widget _buildDateField({
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        InkWell(
          onTap: () => _selectOfficialLeaveDate(isEndDate, firstDate: firstDate),
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              border: Border.all(color: Colors.grey.shade300),
  ...
  Future<void> _selectOfficialLeaveDate(bool isEndDate, {DateTime? firstDate}) async {
    ...
    if (isEndDate && firstDate != null) {
      // For end date: next day of start date + up to 30 calendar days from start date
      startDate = firstDate.add(const Duration(days: 1)); // End date starts from day after start date
      
      // For multiple days: end date can be up to 30 calendar days from start date
      maxDate = firstDate.add(const Duration(days: 30));
      helpText = 'Select End Date (working days only)';
    } else {
      // For start date or single day selection
      if (widget.isSingleDay) {
        // Single day: current date + up to 2 upcoming days (total 3 days) - same as sick leave
        startDate = today;
        maxDate = today.add(const Duration(days: 2));
        helpText = 'Select Official Leave Date (working days only)';
      } else {
        // Multiple days start date: current date + up to 30 calendar days
        startDate = today;
        maxDate = today.add(const Duration(days: 30));
